...
interface TodoProps { newCheckStatus: TodoStatus }
...
const StyledCheckbox = ({ newCheckStatus, className = "", style, ...props }: DivProps & TodoProps) => (
  <div
    ...
    style={{ backgroundColor: newCheckStatus === TodoStatus.COMPLETED ? "#baabb5" : "#e7e8ea", ...(style || {}) }}
    {...props}
  />
);
...
const Text = ({ newCheckStatus, className = "", style, ...props }: React.HTMLAttributes<HTMLSpanElement> & TodoProps) => (
  <span
    className={["font-[Pretendard] text-[16px] max-w-[240px] break-words", newCheckStatus === TodoStatus.COMPLETED ? "line-through" : "", className].join(" ")}
    style={{ color: newCheckStatus === TodoStatus.UNCHECKED ? "#000000" : "#ADADAD", ...(style || {}) }}
    {...props}
  />
);
